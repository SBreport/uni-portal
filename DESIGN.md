...
#### 규칙 2: 1열 우선
- 관련 정보는 **세로로 쌓기** (1열)
- `grid-cols-2` 이상은 "진짜 병렬 비교"인 경우에만
  - ✅ 대시보드 플레이스 vs 웹페이지 (서로 비교)
  - ❌ "지점 정보" 1개 카드를 2열로 쪼개기

#### 규칙 3: 밀집 기본값
- 카드 내부 `p-3` (기본) → 필요 시 `p-4`, 절대 `p-8`+ 아님
- 섹션 사이 `space-y-3` → 최대 `space-y-4`
- `gap-2` 또는 `gap-3`이 기본, `gap-6` 이상 금지
