...
async def get_branch_report(branch_id: int, user: Annotated[dict, Depends(get_current_user)]):
    """지점별 상세 보고서."""
    # branch 역할은 자기 지점만
    ...
    report = get_branch_report(branch_id)
    if not report:
...
async def export_branch_report(branch_id: int, user: Annotated[dict, Depends(get_current_user)]):
    """지점별 보고서 HTML 내보내기."""
    if user["role"] == "branch" and user["branch_id"] != branch_id:
        raise HTTPException(403, "자기 지점만 조회 가능합니다.")

    from reports.export import render_html_report
    html = render_html_report(branch_id)
