...
from shared.db import get_conn as _shared_get_conn, CAFE_DB, EQUIPMENT_DB, now_str
...
def _migrate_cafe_if_needed():
    ...
    for tbl in cafe_tables:
        try:
            ...
            print(f"[WARN] cafe 마이그레이션 테이블 복사 실패 ({tbl}): {e}")
...
def _get_conn():
    ...
    conn = _shared_get_conn(CAFE_DB, attach={"equip": EQUIPMENT_DB})
...
def upsert_article(branch_period_id: int, article_order: int, **kwargs) -> int:
    ...
    try:
        ...
        if row:
            ...
            values = list(fields.values()) + [now_str(), article_id]
...
def set_published_info(article_id: int, url: str, published_by: str):
    ...
    try:
        conn.execute(
            ...
            (url, now_str(), published_by, article_id),
        )
        conn.commit()
    finally:
        conn.close()
...
def _get_equipment_conn():
    ...
    return _shared_get_conn(EQUIPMENT_DB)
...
def update_sync_log(log_id: int, status: str, total_branches: int, total_articles: int, error_log: str = None):
    ...
    try:
        conn.execute(
            ...
            (status, total_branches, total_articles, error_log, now_str(), log_id),
        )
        conn.commit()
    finally:
        conn.close()
