...
### 목적
현재 Notion 동기화는 로컬/서버 각각 수동 실행 필요. 서버에서 자동 주기 동기화로 전환.

### 현재 구조
```
Notion DB → [수동 버튼] → 로컬 equipment.db
Notion DB → [수동 버튼] → 서버 equipment.db (별도 실행 필요)
```

### 목표 구조
```
Notion DB → [서버 자동 동기화, 매일 1회] → 서버 equipment.db
서버 DB가 항상 최신 → 프론트엔드는 서버 DB만 참조
```

### 구현 범위
| 항목 | 설명 | 우선순위 |
|------|------|----------|
| 서버 스케줄러 | APScheduler 등으로 매일 자동 증분 동기화 | P1 |
| 동기화 상태 표시 | 마지막 동기화 시각 + 결과 대시보드 표시 | P1 |
| 실패 알림 | 동기화 실패 시 로그 기록 + 관리자 알림 | P2 |
