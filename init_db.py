...
def _seed_agency_maps(conn):
    """실행사 매핑 초기 데이터 (app_settings 테이블).

    PlaceView / WebpageView에서 하드코딩되었던 AGENCY_MAP을 DB에 저장.
    이미 존재하면 덮어쓰지 않음 (INSERT OR IGNORE).
    """
    import json
    ...
    # 이미 존재하면 스킵
    c.execute("SELECT COUNT(*) FROM app_settings WHERE key LIKE 'agency_map_%'")
    if c.fetchone()[0] > 0:
        return

    # 플레이스 실행사 매핑 (지점 단축명 → 실행사)
    agency_map_place = {
        # 애드드림즈 (21개)
    ...
    c.execute(
        ...
        ("agency_map_place", json.dumps(agency_map_place, ensure_ascii=False)),
    )
    c.execute(
        "INSERT OR IGNORE INTO app_settings (key, value) VALUES (?, ?)",
        ("agency_map_webpage", json.dumps(agency_map_webpage, ensure_ascii=False)),
    )
    conn.commit()
