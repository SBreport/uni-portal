...
{{
  ...
  "study_type": "메타분석|체계적문헌고찰|RCT|비무작위대조시험|전향적코호트|후향적코호트|환자대조연구|증례시리즈|증례보고|전문가리뷰|내러티브리뷰 중 택1",
  "sample_size": "대상자 수 (예: '120명')",
  "follow_up_period": "추적 관찰 기간 (예: '6개월')",
  "related_devices": ["논문에서 다루는 장비/시술 브랜드명을 영문과 한국어 모두 나열"]
}}
...
6. **근거 수준 기준**: 5=메타분석/체계적문헌고찰, 4=RCT, 3=코호트/비교연구, 2=증례보고/시리즈, 1=전문가의견/리뷰, 0=미분류
"""
...
def analyze_with_claude(text: str) -> dict:
    ...
    prompt = ANALYSIS_PROMPT.format(paper_text=text)
...
def process_pdf(pdf_path: str, device_list: list[dict], treatment_list: list[dict]) -> dict:
    ...
    device_id, device_name = match_device_id(related, device_list)
    treatment_id, treatment_name = match_treatment_id(related, treatment_list)

    if device_name:
        print(f"  장비 매칭: {device_name} (id={device_id})")
    else:
