...
```
논문 PDF → [papers/analyzer.py] → Claude API 분석 → DB 저장 + Word/Excel 출력
                                                         ↓
                                              uni-portal 웹앱에서 조회
```
...
### 2.1 CLI 분석 도구 (`papers/analyzer.py`)
...
# 분석 + DB 저장
python papers/analyzer.py --dir papers/

# 분석만 (DB 저장 안 함)
python papers/analyzer.py --dry-run paper.pdf

# 기존 JSON에서 문서만 재출력
python papers/analyzer.py --export-only paper_results/papers_YYYYMMDD.json
```
...
프롬프트는 `papers/analyzer.py`의 `ANALYSIS_PROMPT` 변수에 정의됨.
